...
pca_result = pca.fit_transform(traindata)
...
def main():
    ...
    for component in components:
        for k in range(1, 21):
            prediction = knn(traindata[:, :component], traindata[:, :component], trainlabel, trainlabel, k, 2)
            acc = accuracy(prediction, trainlabel)
            l = loss(prediction, trainlabel)
            print('Components = ', component, ', neighbors = ', k, ', accuracy = ', acc, ', loss = ', l)
